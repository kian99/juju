...
func Open(info *Info, opts DialOpts) (Connection, error) {
	...
	if !info.SkipLogin {
		if err := loginWithContext(dialCtx, loginProvider, st); err != nil {
...
func loginWithContext(ctx context.Context, loginProvider LoginProvider, st *state) error {
	result := make(chan error, 1)
	go func() {
		if loginProvider == nil {
			result <- errors.New("login provider not specified")
			return
		}
