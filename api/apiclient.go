...
func Open(info *Info, opts DialOpts) (Connection, error) {
	...
	bakeryClient.Client.Transport = &hostSwitchingTransport{
		primaryHost: dialResult.controllerAddr.Host,
	...
	if host == "" {
		host = dialResult.controllerAddr.Host
	}
	...
	st := &state{
		...
		clock:               opts.Clock,
		addr:                dialResult.controllerAddr,
		...
		serverScheme:        "https",
		serverRootAddress:   dialResult.controllerAddr.Host,
...
type dialResult struct {
	...
	// controllerAddr represents the controller's root address.
	// E.g. wss://controller.com/foo
	controllerAddr url.URL
	// addr is the full URL (with no DNS resolution) that was actually dialed.
...
func dialAPI(ctx context.Context, info *Info, opts0 DialOpts) (*dialResult, error) {
	...
	logger.Infof("connection established to %q", dialInfo.addr.String())
	...
	for a.Next() {
		...
		if err == nil {
			return &dialResult{
				conn:           conn,
				addr:           d.addr,
				controllerAddr: d.controllerRoot,
				ipAddr:         d.ipAddr,
				tlsConfig:      tlsConfig,
			}, nil
		}
