...
func (s *sshServerSuite) SetUpMocks(c *tc.C) *gomock.Controller {
	...
	s.authenticator.EXPECT().PublicKeyAuthentication(gomock.Any(), gomock.Any()).Return(true).AnyTimes()
	s.authenticator.EXPECT().PasswordAuthentication(gomock.Any(), gomock.Any()).Return(true).AnyTimes()
	s.authorizer.EXPECT().Authorize(gomock.Any(), gomock.Any()).Return(true).AnyTimes()
	s.proxyFactory.EXPECT().New(gomock.Any(), gomock.Any()).Return(s.proxyHandlers, nil).AnyTimes()
