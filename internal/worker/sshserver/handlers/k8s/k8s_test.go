...
func (s *k8sSuite) TestNewHandlers(c *tc.C) {
	...
	c.Check(err, tc.ErrorMatches, "Kubernetes resolver is required not valid")

	_, err = NewHandlers(destination, stubResolver{}, nil, stubExecutor)
	c.Check(err, tc.ErrorMatches, "logger is required not valid")

	_, err = NewHandlers(destination, stubResolver{}, loggertesting.WrapCheckLog(c), nil)
	c.Check(err, tc.ErrorMatches, "executor is required not valid")
	...
	c.Check(err, tc.ErrorMatches, "destination must be a container target not valid")
}
